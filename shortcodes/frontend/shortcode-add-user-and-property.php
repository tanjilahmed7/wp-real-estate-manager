...
if (!class_exists('Wp_rem_Member_Register_User_And_Property')) {
	...
	public function user_and_property_meta_save_callback() {
		...
		if ($property_id != '') {
		    ...
		    $wp_rem_property_price_oncall_num = isset($_REQUEST['wp_rem_property_oncall_number']) ? $_REQUEST['wp_rem_property_oncall_number'] : '';
				...
				<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
					...
					<div class="field-holder">
						<ul id="wp-rem-dev-gal-attach-sec-' . $property_add_counter . '" class="wp-rem-gallery-holder">
							...
							<div class="field-holder dashboard-element-title">
								<strong>What is your MLM user Name ? </strong>
								<input type="text" name="mlm-username">
								</div>
	...
	public function property_packages() {
		...
		if (is_array($cust_query) && sizeof($cust_query) > 0) {
			...
			foreach ($all_pkgs_arr['property_duration'] as $all_pkgs_list_dur) {
				if ($all_pkgs_list_dur == "Unlimited") {
					$rr = $all_pkgs_list_dur;
				}else{
					$rr = ($all_pkgs_list_dur) . ' ' . wp_rem_plugin_text_srt('wp_rem_property_days');
				}
				
			    $package_table .= '<td><span>' . $rr . '</span></td>';
			}
			$package_table .= '</tr>';
			$package_table .= '<tr>';
		...
		foreach ($get_property_floor_plans as $img_item) {
		    ...
		    <div class="modal fade modal-form" id="add-floor-image-data-<?php echo esc_html($rand_img_id); ?>" tabindex="-1" role="dialog">
		    	...
		    	<div class="modal-content">
		    		...
		    		<h4 class="modal-title" id="faqModalLabel"><?php echo wp_rem_plugin_text_srt('wp_rem_edit_details'); ?></h4>
		    	    </div>
	...
	public function custom_fields_features() {
	    ...
	    $property_id = $_POST['get_property_id'];
	...
	public function wp_rem_property_add_transaction($type = '', $property_id = 0, $package_id = 0, $member_id = '') {
		...
		if ($package_id != '' && $package_id != 0) {
		    ...
		    // calculating_amount
		    $wp_rem_trans_amount += ($package_amount);
