	...
	class Wp_rem_Shortcode_Login_Frontend {
		...
		public function wp_rem_login_shortcode_callback($atts, $content = "") {
			...
			if ( is_user_logged_in() ) {
				...
				if ( $wp_rem_demo_user_login_switch == 'on' ) {
					...
					$output_html .='<div class="cs-demo-login-lable">' . 'Click to login with Guest User';
					$output_html .= '</div>';
		...
		public function wp_rem_registration_tab($rand_id = '') {
			...
			if ( $isRegistrationOn ) {
				...
				$wp_rem_opt_array = array(
					...
					'options' => array(
						'landlord' => wp_rem_plugin_text_srt('wp_rem_member_profile_landlord'),
						'tenant' => wp_rem_plugin_text_srt('wp_rem_member_profile_tenant'),
						'builder' => wp_rem_plugin_text_srt('wp_rem_member_profile_builder'),
						'agency' => 'Agent'
		...
		public function wp_rem_profiletop_menu($uid = '', $header_view = '') {
			...
			if ( is_user_logged_in() ) {
				...
				?>
				<?php if ($user_roles[0] != 'custom-admin') {
				?>				
					...
					<ul>
						<li class="user-dashboard-menu-children">
							<a href="">
								...
								if ( $header_view != 'advance_v2' ) {
									?>
									<span class="user-full-name"><?php echo esc_html($LoginName[0]) ?></span>
							...
							<?php if ( ($user_roles != '' && in_array("wp_rem_member", $user_roles) ) || in_array('administrator', $user_roles) ) {
								...
								<ul>
									...
									$dashboard_url = '';
									$dashboard_url = $this->wp_rem_dashboar_top_menu_url('dashboard=suggested');
							  ...
							  <?php
								...
								if ($member_profile_type == 'landlord' OR $member_profile_type == 'builder' && $wp_rem_user_type == 'team-member') {
									if ( true === Wp_rem_Member_Permissions::check_permissions('tenant_list') ) {
										$tenant_list_url = $this->wp_rem_dashboar_top_menu_url('dashboard=tenant_list');
										if ( isset($tenant_list_url) && $tenant_list_url != '' ) {
											$tenant_list_url = $tenant_list_url;
										} else {
											$tenant_list_url = 'javascript:void(0)';
										}
																		?>

									 <li class="user_dashboard_ajax " id="wp_rem_landlord_tenant_list" data-queryvar="dashboard=tenant_list"><a href="<?php echo wp_rem_allow_special_char($tenant_list_url); ?>"><i class="icon-list2"></i><?php echo 'Tenant List'; ?></a></li>
								<?php } }?>
								...
								if ($member_profile_type == 'landlord' OR $member_profile_type == 'builder' && $wp_rem_user_type == 'team-member' OR $member_profile_type == 'builder' && $wp_rem_user_type == 'supper-admin' OR $member_profile_type == 'agency' && $wp_rem_user_type == 'supper-admin' OR $member_profile_type == 'tenant') {
									...
									if ( true === Wp_rem_Member_Permissions::check_permissions('statistics') ) {
											$statistics_url = $this->wp_rem_dashboar_top_menu_url('dashboard=statistics');
											if ( isset($statistics_url) && $statistics_url != '' ) {
												$statistics_url = $statistics_url;
											} else {
												$statistics_url = 'javascript:void(0)';
											}
									?>

									 <li class="user_dashboard_ajax " id="wp_rem_landlord_statistics" data-queryvar="dashboard=statistics"><a href="<?php echo wp_rem_allow_special_char($statistics_url); ?>"><i class="icon-eye2"></i><?php echo 'Statistics'; ?></a></li>
								<?php } }?>
								...
								if ($member_profile_type == 'landlord' OR $member_profile_type == 'builder' && $wp_rem_user_type == 'team-member' OR $member_profile_type == 'builder' && $wp_rem_user_type == 'supper-admin' OR $member_profile_type == 'agency' && $wp_rem_user_type == 'team-member' OR $member_profile_type == 'agency' && $wp_rem_user_type == 'supper-admin') {
									if ( true === Wp_rem_Member_Permissions::check_permissions('my_questions') ) {
										$my_questions_url = $this->wp_rem_dashboar_top_menu_url('dashboard=my_questions');
										if ( isset($my_questions_url) && $my_questions_url != '' ) {
											$my_questions_url = $my_questions_url;
										} else {
											$my_questions_url = 'javascript:void(0)';
										}
																		?>

									 <!-- <li class="user_dashboard_ajax " id="wp_rem_landlord_my_questions" data-queryvar="dashboard=my_questions"><a href="<?php //echo wp_rem_allow_special_char($my_questions_url); ?>"><i class="icon-new-message"></i><?php //echo 'My Questions'; ?></a></li> -->
								<?php } }?>
								...
								<?php
									if ($member_profile_type == 'landlord' OR $member_profile_type == 'builder' && $wp_rem_user_type == 'team-member') {
										...
										$tenancy_requests_url = $this->wp_rem_dashboar_top_menu_url('dashboard=tenancy_requests');
										if ( isset($tenancy_requests_url) && $tenancy_requests_url != '' ) {
											$tenancy_requests_url = $tenancy_requests_url;
										} else {
											$tenancy_requests_url = 'javascript:void(0)';
										}
									?>

									 <li class="user_dashboard_ajax " id="wp_rem_landlord_tenancy_requests" data-queryvar="dashboard=tenancy_requests"><a href="<?php echo wp_rem_allow_special_char($tenancy_requests_url); ?>"><i class="icon-home"></i><?php echo 'Tenancy Requests'; ?></a></li>
								<?php } }?>
								...
								if ($member_profile_type == 'landlord' OR $member_profile_type == 'builder' && $wp_rem_user_type == 'team-member') {
									if ( true === Wp_rem_Member_Permissions::check_permissions('agents_following') ) {
											$agents_following_url = $this->wp_rem_dashboar_top_menu_url('dashboard=agents_following');
											if ( isset($agents_following_url) && $agents_following_url != '' ) {
												$agents_following_url = $agents_following_url;
											} else {
												$agents_following_url = 'javascript:void(0)';
											}
								?>

									  <!-- <li class="user_dashboard_ajax " id="wp_rem_landlord_agents_following" data-queryvar="dashboard=agents_following"><a href="<?php //echo wp_rem_allow_special_char($agents_following_url); ?>"><i class="icon-list"></i><?php //echo 'Agents I’m following'; ?></a></li> -->
								<?php } }?>
								...
								if ($member_profile_type == 'landlord' OR $member_profile_type == 'builder' && $wp_rem_user_type == 'team-member' OR $member_profile_type == 'tenant') {
									if ( true === Wp_rem_Member_Permissions::check_permissions('make_payment') ) {
										$make_payment_url = $this->wp_rem_dashboar_top_menu_url('dashboard=make_payment');
										if ( isset($make_payment_url) && $make_payment_url != '' ) {
											$make_payment_url = $make_payment_url;
										} else {
											$make_payment_url = 'javascript:void(0)';
										}
									?>

									  <!--  <li class="user_dashboard_ajax " id="wp_rem_landlord_make_payment" data-queryvar="dashboard=make_payment"><a href="<?php //echo wp_rem_allow_special_char($make_payment_url); ?>"><i class="icon-attach_money"></i><?php //echo 'Make Payment'; ?></a></li> -->
								<?php } }?>
								...
								if ($member_profile_type == 'landlord' OR $member_profile_type == 'builder' && $wp_rem_user_type == 'team-member' OR $member_profile_type == 'tenant') {
									if ( true === Wp_rem_Member_Permissions::check_permissions('my_owners') ) {
										$my_owners_url = $this->wp_rem_dashboar_top_menu_url('dashboard=my_owners');
										if ( isset($my_owners_url) && $my_owners_url != '' ) {
											$my_owners_url = $my_owners_url;
										} else {
											$my_owners_url = 'javascript:void(0)';
										}
									?>

									   <li class="user_dashboard_ajax " id="wp_rem_landlord_my_owners" data-queryvar="dashboard=my_owners"><a href="<?php echo wp_rem_allow_special_char($my_owners_url); ?>"><i class="icon-user"></i><?php echo 'My Owners'; ?></a></li>
								<?php } }?>

								<?php
								if ($member_profile_type == 'landlord' OR $member_profile_type == 'builder' && $wp_rem_user_type == 'team-member' OR $member_profile_type == 'tenant') {
									if ( true === Wp_rem_Member_Permissions::check_permissions('property_maintenance_requests') ) {
										$property_maintenance_requests_url = $this->wp_rem_dashboar_top_menu_url('dashboard=property_maintenance_requests');
										if ( isset($property_maintenance_requests_url) && $property_maintenance_requests_url != '' ) {
											$property_maintenance_requests_url = $property_maintenance_requests_url;
										} else {
											$property_maintenance_requests_url = 'javascript:void(0)';
										}
									?>

									   <li class="user_dashboard_ajax " id="wp_rem_landlord_maintenance_requests" data-queryvar="dashboard=property_maintenance_requests"><a href="<?php echo wp_rem_allow_special_char($property_maintenance_requests_url); ?>"><i class="icon-business_center"></i><?php echo 'Property Maintenance Requests'; ?></a></li>
								<?php } }?>
								...
								if ($member_profile_type == 'landlord' OR $member_profile_type == 'builder' && $wp_rem_user_type == 'team-member') {
									if ( true === Wp_rem_Member_Permissions::check_permissions('preferences') ) {
										$preferences_url = $this->wp_rem_dashboar_top_menu_url('dashboard=preferences');
										if ( isset($preferences_url) && $preferences_url != '' ) {
											$preferences_url = $preferences_url;
										} else {
											$preferences_url = 'javascript:void(0)';
										}
									?>

									   <!-- <li class="user_dashboard_ajax " id="wp_rem_landlord_preferences" data-queryvar="dashboard=preferences"><a href="<?php //echo wp_rem_allow_special_char($preferences_url); ?>"><i class="icon-address-book"></i><?php //echo 'Preferences'; ?></a></li> -->
								<?php } }?>
								...
								if ($member_profile_type == 'landlord' OR $member_profile_type == 'builder' && $wp_rem_user_type == 'team-member'OR $member_profile_type == 'builder' && $wp_rem_user_type == 'supper-admin'OR $member_profile_type == 'agency' && $wp_rem_user_type == 'supper-admin') {
									if ( true === Wp_rem_Member_Permissions::check_permissions('my_flags') ) {
										$my_flags_url = $this->wp_rem_dashboar_top_menu_url('dashboard=my_flags');
											if ( isset($my_flags_url) && $my_flags_url != '' ) {
												$my_flags_url = $my_flags_url;
											} else {
												$my_flags_url = 'javascript:void(0)';
											}
									?>

									   <li class="user_dashboard_ajax " id="wp_rem_landlord_flag" data-queryvar="dashboard=my_flags"><a href="<?php echo wp_rem_allow_special_char($my_flags_url); ?>"><i class="icon-minus"></i><?php echo 'My Flag'; ?></a></li>
								<?php } }
								...
								if ($member_profile_type == 'builder' && $wp_rem_user_type == 'team-member'OR $member_profile_type == 'builder' && $wp_rem_user_type == 'supper-admin' OR $member_profile_type == 'agency' && $wp_rem_user_type == 'team-member'OR $member_profile_type == 'agency' && $wp_rem_user_type == 'supper-admin') {
									...
									$report_url = $this->wp_rem_dashboar_top_menu_url('dashboard=report');
									...
									?>
									   <li class="user_dashboard_ajax " id="wp_rem_report" data-queryvar="dashboard=report"><a href="<?php echo wp_rem_allow_special_char($report_url); ?>"><i class="icon-tools2"></i><?php echo 'Report'; ?></a></li>
								<?php } }
								?>

								 <?php
								 if ($member_profile_type == 'landlord' && $wp_rem_user_type == 'supper-admin' OR $member_profile_type == 'builder' && $wp_rem_user_type == 'supper-admin'OR $member_profile_type == 'agency' && $wp_rem_user_type == 'supper-admin') {
										$add_property_manager_url = $this->wp_rem_dashboar_top_menu_url('dashboard=add_property_manager');
										if ( isset($add_property_manager_url) && $add_property_manager_url != '' ) {
											$add_property_manager_url = $add_property_manager_url;
										} else {
											$add_property_manager_url = 'javascript:void(0)';
										}
								?>
									<li class="user_dashboard_ajax " id="wp_rem_add_property_manager" data-queryvar="dashboard=add_property_manager"><a href="<?php echo wp_rem_allow_special_char($add_property_manager_url); ?>"><i class="icon-plus"></i>Add Property Manager</a></li>
								 <?php
									}
								?>
								 ...
								 if ($member_profile_type == 'individual') {
										$role_choose_url = $this->wp_rem_dashboar_top_menu_url('dashboard=role_choose');
										if ( isset($role_choose_url) && $role_choose_url != '' ) {
											$role_choose = $role_choose_url;
										} else {
											$role_choose = 'javascript:void(0)';
										}
								?>
									<li class="user_dashboard_ajax " id="wp_rem_member_role_choose" data-queryvar="dashboard=role_choose"><a href="<?php echo wp_rem_allow_special_char($role_choose); ?>"><i class="icon-plus"></i>Choose Your Role</a></li>
								 <?php
									}
								?>

								<?php
								if ($wp_rem_user_type == 'property-member') {?>
									<?php
										$view_properties_assigned_url = $this->wp_rem_dashboar_top_menu_url('dashboard=view_properties_assigned');
										if ( isset($view_properties_assigned_url) && $view_properties_assigned_url != '' ) {
											$view_properties_assigned_url = $view_properties_assigned_url;
										} else {
											$view_properties_assigned_url = 'javascript:void(0)';
										}

										$view_maintenance_requests_url = $this->wp_rem_dashboar_top_menu_url('dashboard=view_maintenance_requests');
										if ( isset($view_maintenance_requests_url) && $view_maintenance_requests_url != '' ) {
											$view_maintenance_requests_url = $view_maintenance_requests_url;
										} else {
											$view_maintenance_requests_url = 'javascript:void(0)';
										}

										$manage_communications_url = $this->wp_rem_dashboar_top_menu_url('dashboard=manage_communications');
										if ( isset($manage_communications_url) && $manage_communications_url != '' ) {
											$manage_communications_url = $manage_communications_url;
										} else {
											$manage_communications_url = 'javascript:void(0)';
										}
									 ?>
										<li class="user_dashboard_ajax" id="wp_rem_member_view_properties_assigned" data-queryvar="dashboard=view_properties_assigned"><a href="<?php echo wp_rem_allow_special_char($view_properties_assigned_url); ?>"><i class="icon-list"></i> View properties assigned</a></li>

										<li class="user_dashboard_ajax" id="wp_rem_member_view_maintenance_requests" data-queryvar="dashboard=view_maintenance_requests"><a href="<?php echo wp_rem_allow_special_char($view_maintenance_requests_url); ?>"><i class="icon-eye"></i>View Maintenance requests</a></li>
