...
jQuery(document).ready(function ($) {
    ...
    jQuery(document).on("click", ".user_dashboard_ajax", function () {
        ...
        var membertype = jQuery(this).attr("data-membertype");
...
console.log(actionString);
        ajaxRequest = jQuery.ajax({
            type: "POST",
            url: wp_rem_globals.ajax_url,
            data: "page_id_all=" + pageNum+"&membertype="+membertype + "&action=" + actionString + filter_parameters,
            success: function (response) {
                jQuery(".user-holder").html('');
...
jQuery( document ).ajaxComplete(function() {
  var x= document.getElementsByClassName('user-holder');
  var y = x[0].lastChild;
  y.nodeValue='';
});
...
jQuery( document ).load(function() {
  var list = document.getElementById("property_detail_sidebar_map_5910");
