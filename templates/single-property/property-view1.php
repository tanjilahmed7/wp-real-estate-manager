    ...
    <div class="page-section property-detail-view1-section">
        <div class="property-detail">
			...
			?>
             ...
             global $wpdb;
             $results = $wpdb->get_results( "SELECT * FROM {$wpdb->prefix}tenant_list WHERE tenant_id = $userid and property_id=$PropertyId", OBJECT );

             if (!empty((array) $results)) {
               $previous_req= true;
               echo '<span class="badge badge-success">You have a already applied for tenant</span>';
             }else{
               $previous_req=false;
             }

             if ( is_user_logged_in() && $wp_rem_property_type=='to-rent' ||(is_user_logged_in() && $wp_rem_property_type=='to-rent'&& $previous_req==false ) ) {
            ?>

              <div class="tenant">
                  <a href="javascript:void(0)" id="tenant-request" class="btn btn-primary " data-toggle="modal" data-target="#Req-tenant">Requst Tenant</a>
                <!-- </form> -->
                <!-- Modal -->
                  <div class="modal fade" id="Req-tenant" tabindex="-1" role="dialog" aria-labelledby="Req-tenantTitle" aria-hidden="true">
                    <div class="modal-dialog modal-dialog-centered" role="document">
                      <div class="modal-content">
                        <div class="modal-header">
                          <h5 class="modal-title" id="Req-tenantTitle">Confirm Request</h5>
                          <h5 class="modal-title" id="Req-tenantTitle">Conform Request</h5>
                        ...
                        <div class="modal-body">
		                  ...
		                  <input type="hidden" id="tenant_request_id" name="tenant_id" value="<?php echo $CurrentUser; ?>">
		                  <input type="hidden" id="property_owner_id" name="property_owner_id" value="<?php echo $author_id; ?>">
		                  <input type="hidden" id="property_id" name="property_id" value="<?php echo $PropertyId; ?>">
		                  <input type="hidden" id="status" name="status" value="0">

                          You are sending tenant request to the ownwer of the prpoperty
                        </div>
                        <div class="modal-footer">
                          <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
                          <button type="button" class="btn btn-primary" id="tenant-req-confirm" data-dismiss="modal" data-backdrop="false">Confirm</button>
                          </form>
                        </div>
                      </div>
                    </div>
